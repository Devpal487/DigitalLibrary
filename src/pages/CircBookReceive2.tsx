...
export default function CircBookReceive2() {
  const Userid = getId();
  ...
  const fetchZonesData = async (data: any) => {
    try {
      // const collectData = {
      //   appId: menuId,
      //   appName: menuName,
      ...
      // };
      // const response = await api.post(
      //   `api/Transaction/MembBalanceData`,
      //   collectData
      // );
      // const data = response.data.data;
      const zonesWithIds = data?.vBookIssdRec?.map((zone: any, index: any) => ({
        ...zone,
        serialNo: index + 1,
        id: index + 1,
      }));
  ...
  const ReceivedData = () => {
    const collectData = {
      ...
      instId: parseInt(instId),
      isRfid: false,
      memberCode: isUserCode,
      accns: [selectedRows[0].accno],
      antNum: 0,
      asOn: new Date().toISOString().slice(0, 10),
    };
    api.post(`api/Transaction/ItemTransaction`, collectData).then((res) => {
